...
const string directoryPath = "./history/";
string uname;
...
list<string> history::get_all()
{
    ...
    all_his.open(directoryPath + all_h_file, ios::in);
    if (!all_his.is_open())
    {
        lmsg = "用户名单：无法打开文件";
        wlog::log(lmsg);
        ...
        // return all_users;
        all_his.close();
        all_his.open(directoryPath + all_h_file, ios::out);
        all_his.close();
        all_his.open(directoryPath + all_h_file, ios::in);
    }
...
void history::app_user(string name)
{
    ...
    all_his.open(directoryPath + all_h_file, ios::app);
    if (!all_his.is_open())
    {
        lmsg = "用户名单：无法写入文件";
        wlog::log(lmsg);
